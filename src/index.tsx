...
app.get('/', (c) => {
  ...
  return c.render(
    <div class="space-y-8">
      ...
      {/* プロレベル一括実行セクション */}
      <div class="bg-gradient-to-r from-purple-50 to-indigo-50 rounded-lg shadow-lg p-6 border border-purple-300">
        ...
        <p class="text-sm text-gray-600 mb-4">
          <i class="fas fa-info-circle mr-1"></i>
          全生徒のプロレベル評価を自動バッチで実行します。すでに今月評価済みの生徒はスキップします。
        </p>
        <div class="flex items-end gap-4">
          <div class="flex-1">
            <label for="prolevel-batch-month" class="block text-sm font-medium text-gray-700 mb-2">
              評価対象月
            </label>
            <input
              type="month"
              id="prolevel-batch-month"
              value={currentMonth}
              class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent"
            />
          </div>
          <button
            id="run-prolevel-batch-btn"
            class="px-6 py-2 bg-gradient-to-r from-purple-600 to-indigo-600 text-white font-bold rounded-lg hover:from-purple-700 hover:to-indigo-700 transition shadow-md whitespace-nowrap">
            <i class="fas fa-rocket mr-2"></i>
            全生徒を一括実行
          </button>
        </div>
        <div id="prolevel-batch-progress" class="hidden mt-4">
          <div class="flex items-center justify-between mb-1">
            <span class="text-sm font-medium text-purple-700" id="prolevel-batch-status">実行中...</span>
            <span class="text-sm font-medium text-purple-700" id="prolevel-batch-count">0 / 0</span>
          </div>
          <div class="w-full bg-gray-200 rounded-full h-3 mb-2">
            <div id="prolevel-batch-bar" class="bg-purple-600 h-3 rounded-full transition-all duration-300" style="width:0%"></div>
          </div>
          <p class="text-xs text-gray-500" id="prolevel-batch-detail">バッチ処理中...</p>
        </div>
      </div>
...
app.post('/api/auto-evaluate', async (c) => {
  try {
    ...
    const skipWanamiUsage = c.req.query('skipWanamiUsage') === 'true' // わなみさん使用回数の取得をスキップ（高速化）
    ...
    console.log(`[Auto Evaluate] Batch size: ${batchSize}, Batch index: ${batchIndex}, Skip pro-level: ${skipProLevel}, Skip X: ${skipX}, Skip YouTube: ${skipYouTube}, Skip Wanami: ${skipWanamiUsage}`)
    if (targetStudentId) {
    ...
    // わなみさん使用回数を取得（プロレベル評価が必要かつスキップされていない場合のみ）
    const wanamiUsageMap = (skipProLevel || skipWanamiUsage) ? new Map<string, number>() : await fetchWanamiUsageCount(GOOGLE_SERVICE_ACCOUNT, month)
    if (!skipProLevel && !skipWanamiUsage) {
      console.log(`[Auto Evaluate] Fetched Wanami usage count for ${wanamiUsageMap.size} students`)
    } else if (skipWanamiUsage) {
      console.log(`[Auto Evaluate] Skipping Wanami usage count (skipWanamiUsage=true)`)
    }
